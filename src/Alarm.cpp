...
bool Alarm::hasVoltAlarm() {
  return voltAlarm;
}

bool Alarm::hasSignalAlarm() {
  return signalAlarm;
}

void Alarm::setMinVolts(uint8_t *min) {
  minVolts = min;
}

void Alarm::setMinSignals(uint8_t *min) {
  minSignals = min;
}

Alarm::Alarm(Audio *audio): audio(audio), last(0), minVolts(0),
    minSignals(0), count(0), voltAlarm(false),
    signalAlarm(false) {
}
